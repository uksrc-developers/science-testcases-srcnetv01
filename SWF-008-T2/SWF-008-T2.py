...
def main():
    ...
    data_path = base_path +"/" # where to find the downloaded data - here, on Azimuth
    ...
    result_path = os.path.join(result_path, "SWF-008-T2")
    ...
    logging.basicConfig(
        filename=f'{result_path}profiling/SWF-008-T2.log',
        filemode='w',
        level=logging.INFO,
        format="%(message)s",
    )
    logger.info(f"{hydra_pspec.__version__ = }")
    logger.info('\nStarted running the workflow.')

    # Load data and extract relevant metadata
    uvd_eor = UVData()
    uvd_eor.read(f"{data_path}eor.uvfits", ant_str="cross")
    uvd_eor.unproject_phase()
    uvd_eor.conjugate_bls()
    uvd_eor = form_pseudo_stokes_vis(uvd_eor)
    
    uvd_fgs = UVData()
    uvd_fgs.read(f"{data_path}foregrounds.uvfits", ant_str="cross")
    uvd_fgs.unproject_phase()
    uvd_fgs.conjugate_bls()
    uvd_fgs = form_pseudo_stokes_vis(uvd_fgs)
    
    logger.info(f"{uvd_eor.Nbls   = }")
    logger.info(f"{uvd_eor.Ntimes = }")
    logger.info(f"{uvd_eor.Nfreqs = }")
    ...
    fig.tight_layout()
    fig.savefig(save_path+"figure-1.pdf")
    logger.info(f'{save_path}figure-1.pdf figure saved.')
    ...
    ax.set_yscale("log")
    fig.tight_layout()
    fig.savefig(f"{save_path}figure-2.pdf")
    logger.info(f"{save_path}figure-2.pdf figure saved.")
    ...
    fig.savefig(f"{save_path}figure-3.pdf")
    logger.info(f"{save_path}figure-3.pdf figure saved.")
    ...
    fig.tight_layout()
    fig.savefig(f"{save_path}figure-4.pdf")
    logger.info(f"{save_path}figure-4.pdf figure saved.")
    ...
    ax.grid()
    fig.tight_layout()
    fig.savefig(f"{save_path}figure-5.pdf")
    logger.info(f"{save_path}figure-5.pdf figure saved.")
    ...
    for ax in fig.axes:
        ax.grid()
    gs.tight_layout(fig)
    fig.savefig(f"{save_path}figure-6.pdf")
    logger.info(f"{save_path}figure-6.pdf figure saved.")
    ...
    fig.tight_layout()
    fig.savefig(f"{save_path}figure-7.pdf")
    logger.info(f"{save_path}figure-7.pdf figure saved.")
    ...
    for seed, antpair in zip(random_seeds, antpairs):
        ...
        logger.info("Generating power spectrum samples...")
        if verbose:
            logger.info()
        start = time.time()
    ...
    fig.savefig(f"{save_path}figure-8.pdf")
    logger.info(f"{save_path}figure-8.pdf figure saved.")
    ...
    fig.savefig(f"{save_path}figure-9.pdf")
    logger.info(f"{save_path}figure-9.pdf figure saved.")
    ...
    for ax in axs:
        ax.grid()
    fig.tight_layout()
    fig.savefig(f"{save_path}figure-10.pdf")
    logger.info(f"{save_path}figure-10.pdf figure saved.")
